...
from PyQt6.QtCore import QUrl  # ← QUrl está en QtCore
...
class RegistroAlquileresTabModern(QWidget):
    ...
    Características:
    - Diseño moderno con tema "Tierra & Asfalto"
    - Filtros avanzados (fecha, cliente, equipo, operador, estado)
    - Tabla con acciones inline
    - Soporte para modalidades de facturación (horas, volumen, fijo)
    - Menú contextual completo
    - Ver conduces con URLs firmadas
    - Marcar pagado/pendiente con validación
    - Copiar celda/fila
    - Integración con Firebase y Storage
    """
    ...
    def _setup_ui(self):
        ...
        # Layout principal SIN TopBar (ya está en app_gui_qt)
    ...
    def _crear_tabla(self):
        ...
        self.tabla.setHorizontalHeaderLabels([
            "Fecha", "Cliente", "Equipo", "Operador", "Ubicación",
            "Cantidad", "Precio", "Monto", "Estado", "C 📄"  # ✅ CAMBIAR: "Conduce" → "C 📄"
        ])
        
        # Configuración
        header = self.tabla.horizontalHeader()
        self.tabla.verticalHeader().setDefaultSectionSize(45)  # 50px de altura por fila
        ...
        header.setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents)  # Ubicación
        ...
        # ✅ Estado: REDUCIR de 130 → 110px (badges más pequeños)
        header.setSectionResizeMode(8, QHeaderView.ResizeMode.Fixed)
        header.resizeSection(8, 150)

        # ✅ Conduce: REDUCIR de 80 → 50px (solo icono)
        ...
        # ✅ Estilo con selección mejorada Y sin fondo en widgets
        self.tabla.setStyleSheet(f"""
            ...
            /* ✅ Forzar transparencia en widgets dentro de celdas */
    ...
    def actualizar_mapas(self, mapas: Dict[str, Dict[str, str]]):
        ...
        También inicializa las fechas dinámicamente.
        """
    ...
    def _inicializar_fechas_filtro(self):
        ...
        Fecha "Desde" = primera transacción
        Fecha "Hasta" = hoy
        """
        try:
    ...
    # =========================================================================================
    # SECCIÓN: Helpers de modalidad para formatear Cantidad y Precio
    # =========================================================================================
    ...
    def _formatear_cantidad_y_precio(self, alquiler: Dict[str, Any]) -> tuple:
        ...
        Modalidades:
        - horas: cantidad = "NN.NN h", precio = precio_por_hora
        - volumen: cantidad = "NN.NN <unidad>", precio = precio_por_unidad
        - fijo: cantidad = "-", precio = monto_fijo (si mostrar_precio_en_modalidad_fijo)
        """
    ...
    # =========================================================================================
    # SECCIÓN: Carga de alquileres desde Firebase
    # =========================================================================================
    
    def _cargar_alquileres_desde_firebase(self):
        """Carga alquileres desde Firebase sin filtros de UI (solo fechas)"""
    ...
    def _aplicar_filtros(self):
        """Aplica los filtros de UI sobre alquileres_cargados"""
    ...
    def _actualizar_tabla(self):
        ...
        for row, alq in enumerate(self.alquileres_filtrados):
            ...
            # Columna 4: Ubicación
            self.tabla.setItem(row, 4, QTableWidgetItem(alq.get('ubicacion', '')))
            
            # Columna 5 y 6: Cantidad y Precio (según modalidad)
            ...
            # Columna 9: Conduce (con icono diferenciado)
            conduce_texto = (alq.get('conduce') or '').strip()
            ...
            tiene_conduce = bool(conduce_texto or conduce_url or storage_path)
    ...
    def _marcar_pagado(self, alquiler: Dict[str, Any]):
        ...
        try:
            # Sumar pagos reales de la subcolección
            pagos_docs = (
            ...
            # Advertir si los pagos no cubren el monto
    ...
    def _accion_toggle_pagado(self):
        ...
        try:
            ...
            # Si se va a marcar como pagado, validar con pagos reales
            if nuevo_estado:
                # Misma lógica que _marcar_pagado
                pagos_docs = (
                    self.fm.db.collection("alquileres")
                    .document(alquiler_id)
                    .collection("pagos")
    ...
    def _generar_url_firmada_fresca(self, storage_path: str, dias: int = 7) -> str:
        ...
        try:
            ...
            # Intentar métodos en inglés/español
    ...
    def _handle_cell_click(self, row: int, col: int):
        """
        Maneja el click en celdas especiales.
        Si el usuario hace click en "Ver Conduce" (columna 9), abre el conduce.
        """
        try:
            # ✅ CAMBIAR: columna 9 ahora (antes era 10)
            if col != 9:
                return
            ...
            # Reutilizar la acción del menú contextual
